...
// - 특정 URL 패턴에 대해서 컴포넌트를 각각 다르게 랜더링하고자 한다면 App.tsx에 <Routes>와 <Route> 컴포넌트를 사용하여 경로에 따라 컴포넌트를 랜더링할 수 있음

// <Routes> : <Route> 컴포넌트로 URL에 따른 컴포넌트 랜더링을 할 수 있도록 하는 컴포넌트
// <Route> : URL 패턴에 따라 랜더링하고자 하는 컴포넌트를 지정하는 컴포넌트
// - path 속성 : URL 패턴을 지정
  ...
  return (
    <Routes>
      ...
      <Route path='/component' element={<Layout/>}>
        ...
        <Route path='curly-braces' element={<CurlyBraces/>}/>   {/* 주의) / 붙이지 않기! */}
      </Route>
      <Route path='/router'>
        {/* localhost:3000/router/query-string?name=홍길동&age=23 */}
